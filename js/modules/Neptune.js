 ...
 * Déssine un triangle
 */
export class Neptune extends AbstractForm {
  ...
  Neptune(ctx, dx, dy) {
   ...
   for (let i = 0; i <= ~~(Math.random() * 59); i++)
      ...
      ctx.ellipse(ox + 50, oy + 325-150 , 46, 9, ~~(Math.random() * 59), ~~(Math.random() * 59), Math.PI * 1);
  ...
  static buildForms() {
    ...
    forms.push(new Neptune( ~~(Math.random() * 1100 ), 100, 10, 10, '', 'white', 1, false, ~~(Math.random() * 9)))
